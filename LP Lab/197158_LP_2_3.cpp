...
void removeleftrecursion()
{
    ...
    vector<string> nonterminal =
        vector<string>(nonterminals.begin(), nonterminals.end());
        ...
        if (grammar.count(nt) > 0 && grammar[nt].size() > 0)
        {
            for (int j = 0; j < i; j++)
            {
                ...
                for (int m = 0; m < rules.size(); m++)
                {
                    ...
                    if (rule.size() > 0 && rule[0] == prev &&
                        grammar.count(prev) > 0 &&
                        grammar[prev].size() > 0)
                    {
                        rules.erase(rules.begin() + m);
                        rule.erase(rule.begin());
                        for (auto other : grammar[prev])
                        {
                            other.insert(other.end(), rule.begin(), rule.end()), addrule(nt, other);
                        }
                        m--;
                    }
                }
